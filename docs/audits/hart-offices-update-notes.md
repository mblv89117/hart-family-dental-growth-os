...
## 6. Yucca Valley Google Business Profile — missing, not invented

No Google Business Profile URL was supplied for Yucca Valley in `Hart Offices.docx`. Desert Hot
Springs has one (`https://share.google/rZOWsM5Bf0Fmiu4yU`). Rather than guessing a URL, a `TODO`
comment is left in `locations.ts` (`social.googleBusinessProfile` for Yucca Valley) and the
Yucca Valley location page / footer simply omit that link until it is supplied.
...
- **TODO:** Yucca Valley Google Business Profile URL — request from the practice and add to
  `locations.ts` (`social.googleBusinessProfile`) once available. Do not invent a placeholder
  link.
